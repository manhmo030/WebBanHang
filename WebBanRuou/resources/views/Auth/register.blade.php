...
<body>
    ...
    <form action="{{ URL::to('/admin/register') }}" method="POST">
        ...
        <div class="form-floating mb-3">
            <input type="text" name="hoten" class="form-control" id="floatingInput">
            <label for="floatingInput">Name: </label>
            @if ($errors->has('tendangnhap'))
                <p class="error-message">{{ $errors->first('hoten') }}</p>
            @endif
        ...
        <button name="login" type="submit" class="btn btn-primary py-3 w-100 mb-4">Sign Up</button>
        <p class="text-center mb-0">Have an Account? <a
                href="{{ URL::to('/admin/') }}">Sign In</a></p>
    </form>
</div>
</div>
</div>
